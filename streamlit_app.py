    ...
    for _, row in stats_df.iterrows():
        ...
        if group_name not in group_color_map:
            ...
            if "vex" in str(curr_mission).lower():
                ...
                group_color_map[group_name] = get_plot_color(
                    curr_mission, experiment_name
                )
        ...
        if st_name == "Nt" and experiment_name == "ec094b":
            ax1.errorbar(
                st_name, snr, label=label, marker="x", markersize=10, color="red"
            )
...
def plot_allan_deviation(data_list):
    ...
    for p in data_list:
        ...
        if taus is not None:
            ax.loglog(
                ...
                oadev,
                ".-",
                label=f"{p.receiving_station_name} ({p.experiment_name})",
                alpha=0.7,
            )
    ...
    oadev_levels = np.logspace(
        np.log10(ax.get_ylim()[0]), np.log10(ax.get_ylim()[1]), 50
    )
...
with st.sidebar:
    ...
    root_dir = st.text_input(
        "Data Root", value="/Users/lgisolfi/Desktop/PRIDE_DATA_NEW/analysed_pride_data"
    )
    all_missions = ["min", "jui", "mex", "mro", "vex"]
    analysis_mode = st.radio(
        "Analysis Scope",
        ["Single Experiment", "Full Mission", "Custom Selection (Global)"],
    )
    ...
    if analysis_mode == "Custom Selection (Global)":
            ...
            if os.path.exists(m_path):
                folders = sorted(
                    [
                        ...
                        if f.startswith(m) and os.path.isdir(os.path.join(m_path, f))
                    ]
                )
                for f in folders:
                    available_options.append((m, f, root_dir))
        options_map = {f"[{m.upper()}] {f}": (m, f, r) for m, f, r in available_options}
        with st.form("global_selector"):
            selected_labels = st.multiselect(
                "Select Experiments", options=list(options_map.keys()), default=[]
            )
            load_btn = st.form_submit_button("Load Selected Data")
        if load_btn:
            st.session_state.processing_queue = [
                options_map[label] for label in selected_labels
            ]
        ...
        if os.path.exists(mission_path):
            available_folders = sorted(
                [
                    ...
                    if f.startswith(selected_mission)
                    and os.path.isdir(os.path.join(mission_path, f))
                ],
                reverse=True,
            )
        if analysis_mode == "Full Mission":
            if st.button("Load Full Mission Data"):
                st.session_state.processing_queue = [
                    (selected_mission, f, root_dir) for f in available_folders
                ]
                st.session_state.data_loaded = True
        else:
            ...
            if selected_folder:
                st.session_state.processing_queue = [
                    (selected_mission, selected_folder, root_dir)
                ]
                st.session_state.data_loaded = True
    ...
    bad_thresh_mhz = (
        st.number_input("Bad Station Limit (mHz)", value=5.0, step=0.5) / 1000.0
    )
...
if st.session_state.data_loaded and st.session_state.processing_queue:
    ...
    with st.spinner("Loading & Filtering Data..."):
        all_raw_data, all_filtered_data = load_and_process_batch(
            queue_tuple, z_threshold
        )
    ...
    def calculate_row(p_data):
        ...
        mean_snr = (
            np.mean(p_data.signal_to_noise) if len(p_data.signal_to_noise) > 0 else 0
        )
        mean_dopp = (
        ...
        return {
            "Mission": p_data.mission_name,
            "Experiment": p_data.experiment_name,
            "Station": p_data.receiving_station_name,
            "Mean SNR (dB)": 10 * np.log10(mean_snr) if mean_snr > 0 else 0,
            "Mean Doppler (mHz)": mean_dopp * 1000,
            "RMS Doppler (mHz)": rms_dopp * 1000,
            "Mean Elevation": mean_el,
            "Status": "❌ Bad" if abs(mean_dopp) > bad_thresh_mhz else "✅ Good",
        }
        ...
        with c_tbl:
            st.dataframe(
                df_stats.style.apply(
                    lambda x: [
                        "background-color: #ffcccc" if "Bad" in x["Status"] else ""
                        for i in x
                    ],
                    axis=1,
                ),
    ...
    with tab2:
        st_names = sorted(
            list(set([d.receiving_station_name for d in all_filtered_data]))
        )
        c1, c2 = st.columns(2)
        sel_stat = c1.selectbox("Select Station", st_names)
        candidates = [
            d for d in all_filtered_data if d.receiving_station_name == sel_stat
        ]
        ...
        if target_data:
            raw_target = next(
                (
                    ...
                    if r.receiving_station_name == target_data.receiving_station_name
                    and r.utc_date == target_data.utc_date
                    and r.mission_name == target_data.mission_name
                ),
                None,
            )
            st.divider()
            ...
            with c_metrics:
                hid = HORIZONS_TARGETS.get(target_data.mission_name, {}).get(
                    "target", "-999"
                )
                times, els, mean_el = get_cached_elevation(target_data, hid)
                st.metric(
                    "Mission / Exp",
                    f"{target_data.mission_name.upper()} / {target_data.experiment_name}",
                )
            ...
            with c_plot:
                compare_mode = st.toggle(
                    "Compare Raw vs Filtered", value=True, key="inspector_toggle"
                )
                if compare_mode and raw_target:
                    st.pyplot(get_comparison_fig(raw_target, target_data))
                else:
                    st.pyplot(get_time_series_fig(target_data))

    with tab3:
        valid_objs = [
            p
            for p in all_filtered_data
            if show_bad or abs(np.mean(p.doppler_noise_hz)) <= bad_thresh_mhz
        ]
        if len(valid_objs) > 12:
            st.info(f"Showing first 12 of {len(valid_objs)} plots.")
        cols = st.columns(3)
        for i, p_data in enumerate(valid_objs[:12]):
            ...
            with cols[i % 3]:
                ...
                ax_g.plot(
                    x, norm.pdf(x, mu, std) * len(noise_mhz) * bin_width, "r--", lw=1
                )
                ax_g.set_title(
    ...
    with tab4:
        ...
        with c_q1:
            target_tau = st.number_input(
                "Query OADEV at Tau (s):", value=10.0, step=10.0
            )
